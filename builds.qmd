...
get_bioc_runiverse_check <- function(y, version) {
    if (y$section %in% c("bioc")) {
        if (version == "release") {
            paste0(
                '<a href="https://bioc-release.r-universe.dev/', 
                y$package, '#checktable">',
                '<img src="https://bioc-release.r-universe.dev/', 
                y$package,
                '/badges/checks" alt="R-universe build status" height="18"></a>'
            )
        } else {
            paste0(
                '<a href="https://bioc.r-universe.dev/', 
                y$package, '#checktable">',
                '<img src="https://bioc.r-universe.dev/', 
                y$package,
                '/badges/checks" alt="R-universe build status" height="18"></a>'
            )
        }
...
make_df <- function(yml) {
    do.call(bind_rows, lapply(yml, function(y) {
        data.frame(
            Package = get_package(y),
            `Bioc devel` = paste(get_bioc_landing_page(y, "devel"), "<br>",
                                 get_bioc_runiverse_check(y, "devel")), 
            `Bioc release` = paste(get_bioc_landing_page(y, "release"), "<br>",
                                   get_bioc_runiverse_check(y, "release")),
...
kable(make_df(yml[vapply(yml, function(y) y$role == "maintainer" && y$section == "bioc", FALSE)]), align = c("l", "c", "c", "c", "c", "c", "c", "c", "c", "c"))
```
...
kable(make_df(yml[vapply(yml, function(y) y$role == "contributor" && y$section == "bioc", FALSE)]), align = c("l", "c", "c", "c", "c", "c", "c", "c", "c", "c"))
```
...
kable(make_df(yml[vapply(yml, function(y) y$section == "bioc-data", FALSE)])[, c("Package", "Bioc devel", "Bioc release", "GitHub Actions", "Code coverage", "GitHub issues", "GitHub PRs", "Bioc support", "Download stats")], align = c("l", "c", "c", "c", "c", "c", "c", "c", "c"))
```
